 ...
 *** modified so that dug responses will get appended. previously, it emptied the `target` HTMLElement before populating with vessel.innerHTML
				...
				dug[callkey] = function(json,cached){
					...
					if( options.target == null ){
						...
						if( options.target.nodeName ){
							...
							var content = Array.prototype.slice.call(vessel.children); //convert HTMLcollection to array
							console.log(content);
							for (var i = 0; i < content.length; i++) {
								document.getElementById(options.target).appendChild(content[i]);							
							}
						}
					}
